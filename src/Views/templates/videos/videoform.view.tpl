<section class="bg-gray-100  rounded-lg shadow-lg p-4 mx-8">
    <h2 class="text-2xl font-bold text-white text-center mb-6"> Video Description {{modedsc}}</h2>
...
{{with video}}
    <form class="my-4 bg-white p-8 rounded shadow-lg mx-auto max-w-md" action="index.php?page=Videos_Videos&mode={{~mode}}&id_video={{id}}" method="POST"><input type="hidden" name="xss_token_video" value="{{~xss_token_video}}"/>{{if ~isADMIN}}<section class="mb-4">
                ...
                <input type="text" id="description_video" name="description_video" placeholder="description_video de video " value="{{description_video}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
                ...
                <input type="text" id="artist_id" name="artist_id" placeholder="artist_id de video " value="{{artist_id}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
                ...
                <input type="text" id="status_video" name="status_video" placeholder="status_video de video " value="{{status_video}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
