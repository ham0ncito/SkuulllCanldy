<section class="bg-gray-100  rounded-lg shadow-lg p-4 mx-8">
    <h2 class="text-2xl font-bold text-white text-center mb-6"> Descripcion album {{modedsc}}</h2>
...
{{with album}}
    ...
    <form class="my-4 bg-white p-8 rounded shadow-lg mx-auto max-w-md" action="index.php?page=Albums_Albums&mode={{~mode}}&id_album={{id}}" method="POST"><input type="hidden" name="xss_token_album" value="{{~xss_token_album}}"/><section class="mb-4">
                ...
                <input type="text" id="id_album" name="id_album" placeholder="id_album de album " value="{{id_album}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
            ...
            <section class="mb-4">
                ...
                <input type="text" id="number_of_songs_album" name="number_of_songs_album" placeholder="number_of_songs_album de album " value="{{number_of_songs_album}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
                ...
                <input type="text" id="id_artist" name="id_artist" placeholder="id_artist de album " value="{{id_artist}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
                {{if id_artist_error}}<div class="text-red-500 text-sm">{{id_artist_error}}</div>{{endif id_artist_error}}
            </section><section class="mb-4">
                <label for="status_album" class="block text-gray-700 text-sm font-bold mb-2">status_album</label>{{endif ~isADMIN}}
                {{if ~isADMIN}}
                <input type="text" id="status_album" name="status_album" placeholder="status_album de album " value="{{status_album}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
                ...
                <input type="text" id="id_genre" name="id_genre" placeholder="id_genre de album " value="{{id_genre}}" {{if ~readonly}}   readonly {{endif ~readonly}} class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-400"/>
